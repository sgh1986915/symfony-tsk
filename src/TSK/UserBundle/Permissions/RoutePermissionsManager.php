...
class RoutePermissionsManager implements PermissionsManagerInterface
{
    private $connection;
    private $container;

    public function __construct(Connection $connection, $container)
    {
    ...
    public function getPermissionsForIdentity($identity, $identityType)
    {
        ...
        foreach ($this->getResources() as $routeId => $route) {
            $defaults = $route->getDefaults();
            if (!empty($defaults['_controller'])) {
                $controller = $defaults['_controller'];
                if (preg_match('/TSK/', $controller)) {
                    $mask = 0;
                    // See if identity has perms on resource
                    foreach ($aces as $idx => $arr) {
                        if ($arr['object_identifier'] == $routeId) {
                            $mask = $arr['mask'];
                            break;
                        }
                    }
